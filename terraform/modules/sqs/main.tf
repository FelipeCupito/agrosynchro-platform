...
# SQS Queue for Drone Data
resource "aws_sqs_queue" "drone_data" {
  name                       = "${var.project_name}-drone-data-queue"
  delay_seconds              = var.delay_seconds
  max_message_size           = var.max_message_size
  message_retention_seconds  = var.message_retention_seconds
  receive_wait_time_seconds  = var.receive_wait_time_seconds
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.drone_dlq.arn
    maxReceiveCount     = var.max_receive_count
  })
...
# Legacy messages queue (mantener por compatibilidad)
...
# Dead Letter Queue for Sensor Data
resource "aws_sqs_queue" "sensor_dlq" {
  name                       = "${var.project_name}-sensor-data-dlq"
...
# Dead Letter Queue for Drone Data
resource "aws_sqs_queue" "drone_dlq" {
  name                       = "${var.project_name}-drone-data-dlq"
  message_retention_seconds  = var.dlq_message_retention_seconds
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          aws_sqs_queue.sensor_data.arn,
          aws_sqs_queue.drone_data.arn,
          aws_sqs_queue.dlq.arn,
          aws_sqs_queue.sensor_dlq.arn,
          aws_sqs_queue.drone_dlq.arn
        ]
      }
    ]
  })
